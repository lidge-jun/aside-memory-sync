...
### 전략 이름에 속지 마라 — `manual` 도 push 한다

`repos.conf` 의 네 번째 필드는 **충돌이 났을 때 뭘 하나**만 정한다. 평상시 동작은 셋 중
둘이 똑같다.

| 전략 | 로컬 변경 | 충돌 | push |
|---|---|---|---|
| `driver` | 자동 커밋 | 병합 드라이버가 처리, 남으면 abort | **한다** |
| `manual` | 자동 커밋 | abort 하고 사람을 부른다 | **한다** |
| `pull` | 건드리지 않는다 | fast-forward 만 받고 아니면 보류 | **안 한다** |

`manual` 은 "사람이 직접 push 한다" 는 뜻이 아니다. 이름이 오해를 부르지만 동작은 위와 같다.
상류를 바꾸면 안 되는 기기 — 예를 들어 읽기용으로만 두는 새 기기, 또는 승인 없이 커밋하면
안 되는 저장소 — 에는 `pull` 을 써라. `pull` 은 커밋도 push 도 하지 않고, 로컬에 변경이
쌓여 있으면 받는 것조차 멈추고 사람을 부른다.
